        ...
        handleSubmit (name) {
          this.$refs[name].validate((valid) => {
            if (valid) {
              ...
              this.$axios.post('http://localhost:9090/leave', {status:this.status, id: this.formValidate.id, reason: this.formValidate.reason, item:this.formValidate.item, time_start: this.dataValue[0],time_end: this.dataValue[1],days:this.formValidate.days})
                .then((response) => {
